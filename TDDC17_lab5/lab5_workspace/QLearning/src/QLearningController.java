	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				// Valuefunction(state_t) = valueFunction(state_t) + learningRate(aka alpha)*(reward + disc_factor*valueFunction(state_t+1) - valueFunction(state_t)) 
				
				double Update = Qtable.get(prev_stateaction) + alpha(Ntable.get(prev_stateaction)) * (previous_reward + GAMMA_DISCOUNT_FACTOR * getMaxActionQValue(new_state) - Qtable.get(prev_stateaction));
				Qtable.put(prev_stateaction, Update);
				/*
				 * a method in predicting a quantity based on future values of given 'signal'.
				 * uses differences in predictions over time to drive the learning process.
				 * Prediction of any given step is updated to bring 
				 * it closer to the prediction of the same quantity 
				 * at the next step.
				 */
				 int nrTicksBeforeStat = 1000; // An example
